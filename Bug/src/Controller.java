...
class Controller extends TimerTask implements MouseListener  {
    ...
    public static final int EFAST_BUG = 2; 
...
//		// create the bugs, now that JFrame has been initialized
    for(int i = 0; i < MAX_NUMBER_OF_BUGS;i++){
        ...
        gameBug[EFAST_BUG][i] = new EFastBug(gameJFrame,1,3);// JFrame, hits required,% change direction    
    }
        //      
  
        //bug level has to be an array
        //for loop
      
       /* 
        gameBug[0] = new SLOW_BUG(gameJFrame,3,10);// JFrame, hits required,% change direction
        gameBug[1] = new FAST_BUG(gameJFrame,4,25);// JFrame, hits required,% change direction 
        gameBug[2] = new HYPER_BUG(gameJFrame,5,30);// JFrame, hits required,% change direction    
        gameBug[3] = new SLOW_BUG(gameJFrame,5,30);// JFrame, hits required,% change direction  
